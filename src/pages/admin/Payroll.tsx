          ...
          <Table head={['Emp no', 'Employee', 'Branch', 'Category', 'Opening balance', 'Accrued', 'Paid out', 'Adjustments', 'Balance', 'Last movement', '']}>
            {withBal.map((b) => (
              <Fragment key={b.employee_id}>
                <tr className={b.balance < -0.005 ? 'bg-red-50 hover:bg-red-100' : 'hover:bg-brand-card'} title={b.balance < -0.005 ? 'Maintenance utilised exceeds the accrual — recover the shortfall' : undefined}><Td>{b.emp_no}</Td><Td>{b.full_name}</Td><Td>{m.bm.code(b.branch_id)}</Td><Td className="text-xs">{b.category}</Td><Td num><Money v={b.opening} /></Td><Td num><Money v={b.accrued} /></Td><Td num><Money v={b.paid_out} /></Td><Td num><Money v={b.adjustments || null} /></Td><Td num className="font-semibold"><Money v={b.balance} /></Td><Td className="text-xs">{fmtDate(b.last_txn)}</Td><Td><Button size="sm" variant="ghost" onClick={() => setOpen(open === b.employee_id ? null : b.employee_id)}>{open === b.employee_id ? 'Hide' : 'Ledger'}</Button></Td></tr>
                {open === b.employee_id && (
                  <tr><Td colSpan={11} className="bg-brand-card">
                    <Table head={['Date', 'Period', 'Type', 'Description', 'Reference', 'Amount']}>
                      {txns.map((t) => { const opened = txns.filter((x) => x.kind === 'opening').map((x) => x.txn_date).sort().pop(); const inside = !!opened && (t.kind === 'opening' ? t.txn_date < opened : t.txn_date < opened); return <tr key={t.id} className={inside ? 'text-slate-400 line-through decoration-slate-300' : ''} title={inside ? 'Dated before the opening balance, so it is already included in it and is not counted again' : undefined}><Td>{fmtDate(t.txn_date)}</Td><Td>{t.period ? periodLabel(t.period) : ''}</Td><Td><Badge tone={t.kind === 'payout' ? 'pink' : t.kind === 'opening' ? 'slate' : 'teal'}>{t.kind}</Badge></Td><Td>{t.description}</Td><Td className="text-xs">{t.reference}</Td><Td num><Money v={t.amount} /></Td></tr> })}
                    </Table>
                  </Td></tr>
                )}
              </Fragment>
