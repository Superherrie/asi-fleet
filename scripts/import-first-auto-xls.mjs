...
const col = { name: c('name'), code: c('code'), driver: c('driver name'), reg: c('reg num'), make: c('make'), model: c('model'), fuel: c('fuel mth sum'), oil_v: c('oil vat'), oil_x: c('oil excl vat'), rep_v: c('repairs vat'), rep_x: c('repairs excl vat'), tyr_v: c('tyres vat'), tyr_x: c('tyres excl vat'), acc_v: c('accident vat'), acc_x: c('accident excl vat'), mnt_v: c('maint serv vat', 'maint vat'), mnt_x: c('maint excl vat'), ovh_v: c('overhaul vat'), ovh_x: c('overhaul excl vat'), oth_v: c('other vat'), oth_x: c('other excl vat'), toll_v: c('toll vat'), toll_x: c('toll excl vat'), exp_v: c('total expenses vat'), exp_x: c('total expenses excl vat'), fee_fixed: c('fixed fee'), fee_lost: c('fee lost card sum'), fee_int: c('fee interest sum'), fee_mag: c('fee magnetic media sum'), fee_txn: c('transaction fee'), fee_scr: c('fee inv scrutiny sum'), fee_vat: c('vat fees levied'), fee_tot: c('total fees'), lost_j: c('lost card journal sum 1'), grand: c('grand total'), odo_c: c('odo close this mth'), litres: c('litre total sum'), odo_p: c('odo prev mth num'), kms: c('kms'), cons: c('consump med mth sum') };
...
for (const r of rows.slice(h + 1)) {
  ...
  const fees_vat = v(r, col.fee_vat); const feeParts = [col.fee_fixed, col.fee_lost, col.fee_int, col.fee_mag, col.fee_txn, col.fee_scr, col.lost_j].reduce((s, i) => s + v(r, i), 0);
  const fees_tot = col.fee_tot >= 0 ? v(r, col.fee_tot) : feeParts + fees_vat; const fees_excl = r2(fees_tot - fees_vat);
  const expenses_excl = col.exp_x >= 0 ? v(r, col.exp_x) : 0; const expenses_vat = col.exp_v >= 0 ? v(r, col.exp_v) : 0; const grand_total = col.grand >= 0 ? v(r, col.grand) : expenses_excl + expenses_vat + fees_tot;
  lines.push({ period, fa_name_code: String(r[col.name] ?? '').trim(), fa_code: String(r[col.code] ?? '').trim(), fa_driver_name: driver, fa_reg: reg, make: String(r[col.make] ?? '').trim(), model: String(r[col.model] ?? '').trim(),
    fuel: v(r, col.fuel), oil_excl: v(r, col.oil_x), oil_vat: v(r, col.oil_v), repairs_excl: v(r, col.rep_x), repairs_vat: v(r, col.rep_v), tyres_excl: v(r, col.tyr_x), tyres_vat: v(r, col.tyr_v), accident_excl: v(r, col.acc_x), accident_vat: v(r, col.acc_v),
    maint_excl: v(r, col.mnt_x), maint_vat: v(r, col.mnt_v), overhaul_excl: v(r, col.ovh_x), overhaul_vat: v(r, col.ovh_v), other_excl: v(r, col.oth_x), other_vat: v(r, col.oth_v), toll_excl: v(r, col.toll_x), toll_vat: v(r, col.toll_v),
    expenses_excl, expenses_vat, fees_excl, fees_vat, grand_total: r2(grand_total), odo_close: nv(r, col.odo_c), odo_prev: nv(r, col.odo_p), kms: nv(r, col.kms), litres: nv(r, col.litres), consumption: nv(r, col.cons) });
}
...
const { data: imp, error: iErr } = await sb.from('fleet_imports').insert({ source: 'first_auto', period, file_name: basename(file), row_count: lines.length, total_amount: sum('grand_total'), notes: 'Combined statement (Monthly Cost Report) — VAT + card fees included; grand total = debit order' }).select('id').single(); if (iErr) throw iErr;
